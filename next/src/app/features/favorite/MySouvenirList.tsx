...
const MySouvenirList = ({ type }: MySouvenirListProps) => {
  ...
  const hasNoSouvenirs = type === 'myPost' ? !myPosts.length : !favoritedSouvenirs.length
  const postsWithMemory = myPosts.filter((post) => post.memory_image_url || post.memory_content)
      ...
      {hasNoSouvenirs ? (
        <Stack spacing={2}>
          {type === 'myPost' ? (
            ...
            <>
              <Text fontSize="sm">お気に入りに登録したお土産はありません。</Text>
              <TextIconLink
                iconName="FaSearch"
                iconPosition="left"
                href="/search"
